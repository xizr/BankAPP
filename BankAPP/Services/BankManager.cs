...
namespace BankAPP.Services
{
    public class BankManager : IBankManager
    {
        ...
        public void DepositCard(User user, string cardNumber)
        {
            foreach (Card card in user.Cards)
            {
                if (card.Number == cardNumber)
        ...
        public void ShowAllBalances(User user)
        {
            Console.WriteLine(user.Balance);
        }
            ...
            foreach(Card card in user.Cards)
            {
                if(card.Number == cardNumber && card.PinCode == pin) {
                    ...
                    if(card.Balance >= pul)
                    {
                        card.Balance -= pul;
                        user.Balance -= pul;
                        Console.WriteLine("Pul yechildi ");
                    }
                    else
                    {
        ...
        public void WithdrawCard(User user, string cardNumber, string pin)
        {
            foreach (Card card in user.Cards)
            {
                if (card.Number == cardNumber && card.PinCode == pin)
